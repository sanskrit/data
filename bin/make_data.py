...
parser.add_argument(
    ...
    action="store_true",
    help="If set, generate prefixed verbals " + "('AgacCati', 'Agata', 'Agantum').",
)
...
def write_prefix_groups(
    prefixed_roots, unprefixed_roots, upasargas, other, sandhi_rules, out_path
):
...
def write_mw_prefixed_roots(
    prefixed_roots, unprefixed_roots, prefix_groups, sandhi_rules, out_path
):
    ...
    for row in util.read_csv_rows(prefixed_roots):
        for group in sandhi.split_off(row["prefixed_root"], row["unprefixed_root"]):
...
def make_root_converter(
    shs_roots_path, shs_blacklist_path, shs_override_path, mw_unprefixed_roots_path
):
...
def write_shs_verbal_indeclinables(adverbs_path, final_path, root_converter, out_path):
...
def write_prefixed_shs_verbal_data(
    data_path, prefixed_roots, root_converter, sandhi_rules, out_path
):
    """Write Sanskrit Heritage Site data after converting its roots.
...
def write_prefixed_shs_verbal_indeclinables(
    final_path, sandhi_rules, prefixed_roots, root_converter, out_path
):
...
def build_data(project_dir, output_dir, make_prefixed_verbals):
    ...
    copy_to_output_dir(
        paths["lso/nominal-endings-compounded"],
        out_path("nominal-endings-compounded.csv"),
    )
    copy_to_output_dir(
        paths["lso/nominal-endings-inflected"],
        out_path("nominal-endings-inflected.csv"),
    )

    heading("Simple indeclinables")
    cat(
        [paths["mw/indeclinables"], paths["lso/indeclinables"]],
    ...
    copy_to_output_dir(paths["mw/unprefixed-roots"], out_path("unprefixed-roots.csv"))
    write_mw_prefixed_roots(
        paths["mw/prefixed-roots"],
        unprefixed_roots=paths["mw/unprefixed-roots"],
    ...
    heading("Verbs")
    root_converter = make_root_converter(
        paths["shs/roots"],
        paths["shs/root-blacklist"],
        paths["shs/root-override"],
        paths["mw/unprefixed-roots"],
    )
    write_shs_verbal_data(
        data_path=paths["shs/roots"],
    ...
    if make_prefixed_verbals:
        ...
        write_prefixed_shs_verbal_data(
            data_path=paths["shs/roots"],
